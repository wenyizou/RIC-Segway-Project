...
//  define for motor status
typedef struct {                      // Define inside the #define mark!!!!!!!!  otherwise error of undefined occur!!!
    BYTE gyro_x[2];         // store 2 bytes of gyro_x data, [0] is high byte, [1] is low bit
    char cgyro_x[6];
    BYTE gyro_y[2];
    char cgyro_y[6];
    BYTE gyro_z[2];
    ...
    int posterior;
    char cposterior[5];
    char command_response[20];        // consist of "/0" and one char indicator
    char motor_position[20];         // operand from 0 to 2^31
    char query_response[40];          // for special response
}MOTOR_STATUS;
